...
Or you can do everything in Atom Editor. For this you need to install [ide-python](https://atom.io/packages/ide-python) (+ enable pylint there), install [python-tools](https://atom.io/packages/python-tools), install `pylint` to each working Python environment and tune it globally: edit or create `~/.pylintrc` file (create on Windows: `echo bla > .pylintrc`):
